...
max_num_frames = 90; %cut off for timelapse%50 for CDC25, 70 for Cdc13
time_int = 5; %minutes
activity_thresh = 0.5;
pixel_size = 0.130;
...
%       nuc_centroids = cell2mat({nuc_stats.Centroid}');
    ...
    if length(cell_length) < (cell_death_frame - cell_born_frame)+1 || max(cell_fluor./nuc_fluor_values) < activity_thresh
        continue
    end
    ...
    CM_index = randi(256,1);
    hold on
    %subplot(1,2,2)
     plot(time_vals/max(time_vals), cell_fluor./nuc_fluor_values,'color','b');
...
%      yyaxis left
    plot(time_vals, cell_fluor_time./nuc_fluor_time);
...
%CDK_smooth = smoothdata(cell_fluor_time./nuc_fluor_time,'movmedian',med_filter_sz);
  figure,
    hold on
    yyaxis left
    p1 = plot(time_vals/max(time_vals), cell_length_time,'b');
    ylabel('Cell Length (um)')
    yyaxis right
    %p2 = plot(time_vals/max(time_vals), cell_fluor_time./nuc_fluor_time, 'b-');
    p2 = plot(time_vals/max(time_vals), cell_fluor_time./nuc_fluor_time, 'r-');
