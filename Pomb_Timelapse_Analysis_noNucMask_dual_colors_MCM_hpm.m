...
%     plot_idx = [nuc_fluor_time(min_elem),nuc_fluor_time(max_elem),nuc_fluor_time(end)];
...
%Protein 2 (Cdc13)
    ...
    nuc_fluor_smooth_2 = smoothdata(nuc_fluor_time_2,'movmedian',3);
    ...
    %ylabel('Cell Length (um)')
    %yyaxis left
    plot(time_vals_2, nuc_fluor_time_2);
...
max_time_frame = 41;
    ...
    CDK_smooth = smoothdata(CDK_activity_time,'sgolay',3);
    fluor_smooth_3 = smoothdata(fluor_time_3,'sgolay',3);
    fluor_smooth_4 = smoothdata(fluor_time_4,'sgolay',3);
    time_vals = ((1:length(fluor_time_3))-1)*time_int;
    %time_vals = ((1:max_time_frame)-1)*time_int;
    norm_Cdc13_Nuc_raw = (fluor_time_3- min( fluor_time_3 ))/(max( fluor_time_3 )-min( fluor_time_3 ));
    norm_MCM_raw = (CDK_activity_time -min(CDK_activity_time))/(max(CDK_activity_time )-min(CDK_activity_time ));
  figure,
    hold on
    %yyaxis left
   ...
   plot(time_vals, fluor_time_5,'Color',[0.4940 0.1840 0.5560],'LineStyle','-','LineWidth',1.0)%
    ylabel('Mean Intensity')
     xlabel('Time from birth (minutes)')
     hold off
%     
    figure, 
     hold on
...
%     ylabel('Cell Length')
    ...
    xlabel('Mean Cdc13 Intensity Normalized (a.u.)')
    ylabel('CDK activity (C/N) Normalized');
    
    hold off
 
%      hold on
%      plot(time_vals, fluor_time_3, time_vals, CDK_activity_time/max(CDK_activity_time))
%     text(fluor_time_3, CDK_activity_time, num2str(cell_length_time));
%     xlabel('Mean Cdc13 Intensity (a.u.)')
%      ylabel('CDK activity (C/N)');
% %     
%     hold off
