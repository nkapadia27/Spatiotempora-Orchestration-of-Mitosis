...
for i=1:num_cells
   ...
   Nuc_MCM = cell2mat(Fluor_cell(i,4));
    ...
    CDK_MCM_smooth = smoothdata(CDK_activity_MCM_time,'sgolay',7);
    Cdc13_Cyto_smooth = smoothdata(Cdc13_Cyto,'sgolay',4);
    Cdc13_Nuc_smooth = smoothdata(Cdc13_Nuc,'sgolay',5);
    ...
    if id_MCM<=time_pts_cut || id_MCM + time_pts_length >=length(cell_length_time)
        continue 
    end
    ...
    cell_length_select = cell_length_time(id_MCM + time_pts_length - time_pts_cut_length:id_MCM+time_pts_length);
  
ipt_Cdc13 = findchangepts(Cdc13_Nuc_Smooth_select, 'Statistic', 'linear','MinThreshold',20,'MinDistance',3);%2 and 4
    ipt_MCM = findchangepts(CDK_MCM_Smooth_select, 'Statistic', 'linear','MinThreshold',0.002,'MinDistance',4);%2 and 4
    ...
    plot((Cdc13_Nuc_smooth-min(Cdc13_Nuc_smooth))/(max(Cdc13_Nuc_smooth)-min(Cdc13_Nuc_smooth)), (CDK_MCM_smooth-min(CDK_MCM_smooth))/(max(CDK_MCM_smooth)-min(CDK_MCM_smooth)))
    xlabel('Mean Normalized Nuclear Cdc13 Intensity (a.u.)')
    ylabel('C/N Ratio Normalized')
    title('Normalized Smooth Trace')
    hold on
    subplot(1,2,1)
...
%     plot(time_vals/max(time_vals),CDK_activity_time_syn/max(CDK_activity_time_syn),'b-', time_vals/max(time_vals), CDK_activity_time_mcm/max(CDK_activity_time_mcm),'r-');
    ...
    text(time_vals(loc_MCM)/max(time_vals), CDK_MCM_smooth(loc_MCM),'Nuclear Activation')
...
%     ylabel('Cell Length')
    ...
    hold off
     ...
     figure, 
    hold on
    plot(Cdc13_Nuc_smooth, CDK_MCM_smooth)
    ...
    figure, 
    yyaxis left
    plot(time_vals/max(time_vals), Cdc13_Nuc_smooth,'m-',time_vals/max(time_vals), Cdc13_Nuc,'m--','LineWidth',1.0)%
     hold on
    yyaxis right
    plot(time_vals/max(time_vals),CDK_MCM_smooth,'r-',time_vals/max(time_vals), CDK_activity_MCM_time,'r--','LineWidth',1.0);
   ylabel('Sensor Activity')
