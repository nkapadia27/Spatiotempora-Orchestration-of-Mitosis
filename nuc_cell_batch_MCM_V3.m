...
max_num_frames = 75; %cut off for timelapse%50 for CDC25, 70 for Cdc13
time_int = 10; %minutes
pixel_size = 0.130;
...
elem_del = [];
    ...
    %         nuc_inten_mean2 = mean(nuc_px_vals);
        ...
        nuc_cell_arr{j,3} = mean(nuc_inten_arr(:,3)); %Mean  Diameter Object nucleus
...
num_cells_fin = length(uniq_cell_arr(:,1));
 figure
 uniq_cell_arr_2 = cell(num_cells_fin,1);
 unique_cells_2 = zeros(num_cells,1);
for kk = 1:num_cells_fin
    ...
    if max(inten_wt_cell./inten_wt_nuc) < activity_thresh
        continue
    end
   hold on
    ...
    plot (time_points/max(time_points), inten_wt_cell./inten_wt_nuc)
    ylim([min_scale,max_scale])
    uniq_cell_arr_2{kk}=uniq_cell_arr{kk};
    unique_cells_2(kk) = unique_cells(kk);
end
hold off
uniq_cell_arr_2 = uniq_cell_arr_2(~cellfun(@isempty, uniq_cell_arr_2(:,1)), :);
uniq_cell_arr = uniq_cell_arr_2;
unique_cells_2 = unique_cells_2(any(unique_cells_2,2),:); 
unique_cells= unique_cells_2;
num_cells_fin = length(uniq_cell_arr);

%% 
%num_cells = length(div_length(:,1));
Fluor_cell = cell(num_cells_fin, 2);%cell_ID, cell_length, cell intensity, nuclear intensity, rate concentration increase, max rate growth


for i =1:num_cells_fin
...
cell_dat = uniq_cell_arr{cell_num};
 cell_length_time = (cell2mat(Fluor_cell(cell_num ,2)));
  ...
  figure,
    hold on
    yyaxis right
   %plot(time_vals/max(time_vals), smoothdata(cell_fluor_time./nuc_fluor_time,'movmedian',med_filter_sz),time_vals/max(time_vals),cell_fluor_time./nuc_fluor_time);
%    hold on 
   plot(time_vals/max(time_vals),cell_length_time);
     ylabel('Cell Length (um)')
    yyaxis left
    %p2 = plot(time_vals, cell_fluor_smooth, 'b-');
%     p3 = plot(time_vals/max(time_vals), cell_fluor_time, 'b-');
%     %p4 = plot(time_vals, nuc_fluor_smooth,'g-');
%     p5 = plot(time_vals/max(time_vals), nuc_fluor_time,'g-');
