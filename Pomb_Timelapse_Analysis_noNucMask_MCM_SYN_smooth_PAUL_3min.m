...
max_num_frames = 120;%120 usually
time_int =3; %minutes
pixel_size = 0.130;
...
time_pts_cut_syn = 15;%default 10
time_pts_length = 15;%default 7
time_pts_length_cut = 17;%default 10
...
for i=1:num_cells
  ...
  %CDK_activity_syn_smooth = smoothdata(CDK_activity_time_syn, 'gaussian',3);
    CDK_activity_mcm_smooth = smoothdata(CDK_activity_mcm, 'sgolay',6);
    cell_length_smooth = smoothdata(cell_length_time, 'sgolay',6);
    ...
    if id_SYN<=time_pts_cut_syn || max_SYN<Syn_thresh 
        continue 
    end
    ...
    if id_MCM + time_pts_length > length(cell_length_time)
     ...
     cell_length_select = cell_length_smooth(id_MCM+time_pts_length - time_pts_length_cut:id_MCM+time_pts_length);
    %ipt_MCM = findchangepts(CDK_MCM_Smooth_select , 'Statistic', 'linear','MinThreshold',0.00065);%1 and 4
    ipt_MCM = findchangepts(CDK_MCM_Smooth_select , 'Statistic', 'linear','MinThreshold',0.0002,'MinDistance',6);%1 and 4
    ipt_SYN = findchangepts(CDK_SYN_Smooth_select, 'Statistic', 'linear','MinThreshold',0.0002,'MinDistance',5);
    ipt_length = findchangepts(cell_length_select , 'Statistic', 'linear','MaxNumChanges',1,'MinDistance',5);% 1 and 4
    ...
    time_scale = (length(cell_length_smooth)-1)*time_int;
    ...
    MCM_SYN (i,4) = cell_length_smooth(loc_MCM);
    MCM_SYN (i,5) = cell_length_smooth(loc_Syn);
    MCM_SYN (i,6) = div_length(i,3) - cell_length_smooth(loc_MCM);
    MCM_SYN (i,7) = div_length(i,3) - cell_length_smooth(loc_Syn);
...
time_pts_cut_mcm = 15;%default 15
time_pts_cut_syn = 15;%default 15
time_pts_length = 15;%default 15
time_pts_length_cut = 17;%default 17
  ...
  %CDK_activity_syn_smooth = smoothdata(CDK_activity_time_syn, 'gaussian',3);
    CDK_activity_mcm_smooth = smoothdata(CDK_activity_time_mcm, 'sgolay',6);
    cell_length_smooth = smoothdata(cell_length_time, 'sgolay',6);
    %fluor_time_3 = cell2mat(Fluor_cell(cell_num ,6));
    %fluor_smooth_3 = smoothdata(fluor_time_3,'movmedian',5);
    ...
    CDK_SYN_Smooth_select = CDK_activity_syn_smooth(id_SYN - time_pts_cut_syn:id_SYN);
    cell_length_select = cell_length_smooth(id_MCM+time_pts_length - time_pts_length_cut:id_MCM+time_pts_length);
     %ipt_MCM = findchangepts(CDK_MCM_Smooth_select , 'Statistic', 'linear','MaxNumChanges',2,'MinDistance',6);%1 and 4
%      ipt_SYN = findchangepts(CDK_SYN_Smooth_select , 'Statistic', 'linear','MaxNumChanges',1,'MinDistance',6);%1 and 4
%     ipt_MCM = findchangepts(CDK_MCM_Smooth_select , 'Statistic', 'linear','MinThreshold',0.00065);%1 and 4
     ipt_MCM = findchangepts(CDK_MCM_Smooth_select , 'Statistic', 'linear','MinThreshold',0.0002,'MinDistance',6);%1 and 4
    ipt_SYN = findchangepts(CDK_SYN_Smooth_select, 'Statistic', 'linear','MinThreshold',0.0002,'MinDistance',5);%1 and 3
    ipt_length = findchangepts(cell_length_select , 'Statistic', 'linear','MaxNumChanges',1,'MinDistance',5);% 1 and 4
    loc_MCM = id_MCM - (time_pts_cut_mcm - ipt_MCM(1) + 1);
    loc_Syn = id_SYN - (time_pts_cut_syn - ipt_SYN(1) + 1);
    loc_length = id_MCM+time_pts_length - (time_pts_length_cut  - ipt_length(1) + 1);
...
%   
    ...
    ylabel('Sensor Activity')
     yyaxis left
    plot(time_vals/max(time_vals),  cell_length_smooth,'b-','LineWidth',1);
...
[b2,Sfit_MCM] = polyfit(MCM_Combined(:,4), MCM_Combined(:,8),1);
[Yfit_MCM, delta_fit_MCM] = polyconf(b2, MCM_Combined(:,8), Sfit_MCM);
mdl_MCM =  fitlm(MCM_Combined(:,4),MCM_Combined(:,8),'linear');
yCalc1_MCM = polyval(b2, MCM_Combined(:,4));
figure(5), 
scatter(MCM_Combined(:,4), MCM_Combined(:,8))
hold on 
plot(MCM_Combined(:,4), yCalc1_MCM );
hold off
